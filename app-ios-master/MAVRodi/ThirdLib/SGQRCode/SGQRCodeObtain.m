...
- (void)establishQRCodeObtainScanWithController:(UIViewController *)controller configure:(SGQRCodeObtainConfigure *)configure {
    ...
    if (configure.rectOfInterest.origin.x == 0 && configure.rectOfInterest.origin.y == 0 && configure.rectOfInterest.size.width == 0 && configure.rectOfInterest.size.height == 0) {
        ...
        self.metadataOutput.rectOfInterest = configure.rectOfInterest;
    }
    ...
    self.metadataOutput.metadataObjectTypes = configure.metadataObjectTypes;
    
    // 6、预览图层
    self.videoPreviewLayer = [AVCaptureVideoPreviewLayer layerWithSession:_captureSession];
    // 保持纵横比，填充层边界
    self.videoPreviewLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;
    self.videoPreviewLayer.frame = controller.view.frame;
...
- (void)startRunningWithBefore:(void (^)(void))before completion:(void (^)(void))completion {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            // 修正扫描区域  M by rttx
            CGFloat w = 0.6 * weakSelf.videoPreviewLayer.frame.size.width;
            CGFloat x = 0.5 * (1 - 0.6) * weakSelf.videoPreviewLayer.frame.size.width;
            CGFloat y = 0.5 * (weakSelf.videoPreviewLayer.frame.size.height - w) - TE_NavigationBar_Height;
            
            CGRect scanRect = CGRectMake(x, y, w, w);
            CGRect insertRect = [weakSelf.videoPreviewLayer metadataOutputRectOfInterestForRect:scanRect];
            weakSelf.metadataOutput.rectOfInterest = insertRect;
            
            // 扫描区域
            CGRect previewScanRect = [weakSelf.videoPreviewLayer rectForMetadataOutputRectOfInterest:insertRect];
            MRLog(@"二维码扫描区域-> %@", NSStringFromCGRect(previewScanRect));
...
- (void)P_enterImagePickerController {
    ...
    // 系统照片选择器处理了UIScrollView的contentInsetAdjustmentBehavior为automatic，
    // 项目里关闭了，需要设置为automatic。
    if (@available(iOS 11, *)) {
