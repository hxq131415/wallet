...
class PlanUnfreezeAccountViewController : MRBaseViewController {
    ...
    lazy var contentView: PlanUnfreeAccountContentView = PlanUnfreeAccountContentView(frame: CGRect.zero)
...
//    var unFreezeamout:Double = 0.00
    ...
    func subViewInitlzation() {
        ...
        navBarView.titleLbl.text = MRLocalizableStringManager.localizableStringFromTableHandle(key: MRPlan_Thaw_account_title_key)
        ...
        contentView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(400 * theScaleToiPhone_6)
...
//        contentView.amountTextField.keyboardType = .numberPad
        ...
        if let userInfo = MRCommonShared.shared.userInfo {
            self.contentView.constTextField.text = String(format: "%.0f", userInfo.unlock_need_mcoin)
        }
        ...
        let title = MRLocalizableStringManager.localizableStringFromTableHandle(key: MRPlan_Thaw_mail_notification_freeze_result_title_key)
...
//        }
        ...
        enterPwdCtrl.enterPwdFinishClosure = { [weak self](pwd) in
            if let weakSelf = self {
                weakSelf.requestForUnfeezeAccount(pwd: pwd)
            }
        }
    ...
    // 解冻账号
    func requestForUnfeezeAccount(pwd: String) {
        
        var params: [String: Any] = [:]
        params["payPassword"] = pwd
        params["gasPrice"] = contentView.constTextField.text
        
        TEActingLoadingView.showSystemActingLoadingViewToSuperView(superView: self.view)
        
        TERequestManager.dataRequestForPOSTWithCode(with: MR_REQ_CODE_PLAN_UNFREEZE_Account, parameters: params, requestInfo: nil) { [weak self](data, status, message) in
            if let weakSelf = self {
                ...
                if status == 0 {
                    if let msg = message, !msg.isEmpty {
                        ...
                        weakSelf.showMessage("付款成功，解冻申请中，请稍后")
