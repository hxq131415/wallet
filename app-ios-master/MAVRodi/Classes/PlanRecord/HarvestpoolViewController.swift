//
//  HarvestpoolViewController.swift
//  MAVRodi
//
...
class HarvestpoolViewController: MRBaseViewController {
    ...
    private var harvesBtn: UIButton = {
        let harvesBtn_ = UIButton(type: UIButtonType.custom)
        return harvesBtn_
    }()
    ...
    func subViewInitlzation() {
        ...
        navBarView.titleLbl.text = MRLocalizableStringManager.localizableStringFromTableHandle(key: HarvestpoolPage_navigation_title_key)
        ...
        centerImgVw.image = UIImage(named: "icon_harvest_pool_rotation")
        ...
        waveView.backWaveColor = UIColorFromRGB(hexRGB: 0x1ed2b0)
        waveView.foreWaveColor = UIColorFromRGB(hexRGB: 0x18a4ac)
        ...
        self.view.addSubview(harvesBtn)
        harvesBtn.snp.makeConstraints { (make) in
        ...
        harvesBtn.backgroundColor = UIColorFromRGB(hexRGB: 0x2abbb0)
        harvesBtn.layer.masksToBounds = true
        harvesBtn.layer.cornerRadius = 48 * theScaleToiPhone_6 / 2
        harvesBtn.setTitle(MRLocalizableStringManager.localizableStringFromTableHandle(key: HarvestpoolPage_bottom_btn_title_key), for: UIControlState.normal)
        harvesBtn.addTarget(self, action: #selector(self.harvesBtnAction), for: UIControlEvents.touchUpInside)
        ...
        bottomLbl.configLabel(text: MRLocalizableStringManager.localizableStringFromTableHandle(key: HarvestpoolPage_game_reminder_text_key), numberOfLines: 0, textAlignment: NSTextAlignment.center, textColor: UIColorFromRGB(hexRGB: 0x16a2ac), font: UIFont.systemFont(ofSize: 14))
    ...
    // MARK: - 立即收获
    @objc func harvesBtnAction() {
        
        let harves = PlanHarvestViewController()
        self.pushToViewController(viewCtrl: harves)
    }
            ...
            if let weakSelf = self {
                ...
                if status == 0 {
                    if let dict = data as? [String: Any] {
                        if let amount = dict["js_amount"] as? Double ,
                            let totalAmount = dict["js_totalAmount"] as? Double {
