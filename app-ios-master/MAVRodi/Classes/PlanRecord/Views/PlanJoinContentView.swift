...
@objc protocol PlanJoinContentViewDelegate: NSObjectProtocol {
    
    // 金额输入发生变化
    @objc func amoutInputChanged(text: String?)
}

class PlanJoinContentView: UIView, UITextFieldDelegate {
    ...
    weak var delegate: PlanJoinContentViewDelegate?
    ...
    func subViewInitialization() {
        ...
        amountTextField.placeholder = MRLocalizableStringManager.localizableStringFromTableHandle(key: PlanJoinPage_left_amount_textfield_placehorder_text_key)
        ...
        amountRightLbl.configLabel(text: "ETC", textAlignment: .right, textColor: UIColorFromRGB(hexRGB: 0x2e2922), font: UIFont.systemFont(ofSize: 15))
        ...
        passwordLbl.snp.makeConstraints { (make) in
            make.left.equalTo(self.minerView)
            make.right.equalTo(self.minerView)
            make.height.greaterThanOrEqualTo(10)
            make.top.equalTo(self.minerView.snp.bottom).offset(20)
        }
        
        passwordLbl.configLabel(text: MRLocalizableStringManager.localizableStringFromTableHandle(key: PlanJoinPage_contentView_bottom_reminder_text_key), textAlignment: NSTextAlignment.left, textColor: UIColorFromRGB(hexRGB: 0x7e7e7e), font: UIFont.systemFont(ofSize: 10))
        ...
        joinBtn.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(passwordLbl.snp.bottom).offset(24 * theScaleToiPhone_6)
        }
        
        joinBtn.configButtonForNormal(title: MRLocalizableStringManager.localizableStringFromTableHandle(key: PlanJoinPage_bottom_btn_title_key), textColor: UIColorFromRGB(hexRGB: 0xffffff), font: UIFont.systemFont(ofSize: 19), normalImage: nil, selectedImage: nil, backgroundColor: UIColorFromRGB(hexRGB: 0x33c6b3))
