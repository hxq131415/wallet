//
//  WalletExportPublicKeyViewController.swift
//  MAVRodi
//
...
class WalletExportPublicKeyViewController: MRBaseViewController {

    private lazy var mainTableView: UITableView =
        UITableView(frame: CGRect.zero, style: UITableViewStyle.plain)
    
    var publicKeyModel: CreateWalletPrivateKeyModel!
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = MRLocalizableStringManager.localizableStringFromTableHandle(key: ExportPublicKeyPage_navigation_title_key)
...
extension WalletExportPublicKeyViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if let headerView = Bundle.main.loadNibNamed("ExportPublicKeyContentView", owner: nil, options: nil)?.first as? ExportPublicKeyContentView {
            
            headerView.frame = CGRect(x: 0, y: 0, width: Main_Screen_Width, height: 594)
            
            headerView.ethPrivateKeyLbl.text = publicKeyModel.eth_privateKey
            headerView.etcPrivateKeyLbl.text = publicKeyModel.etc_privateKey
            ...
            headerView.ethCopyBtnTapClosure = { [weak self] in
                if let weakSelf = self {
                    let pasteBoard = UIPasteboard.general
                    pasteBoard.string = headerView.ethPrivateKeyLbl.text
                    weakSelf.showMessage(MRLocalizableStringManager.localizableStringFromTableHandle(key: MR_copyed_prompt_text_key))
                }
            }
            
            headerView.etcCopyBtnTapClosure = { [weak self] in
                if let weakSelf = self {
                    let pasteBoard = UIPasteboard.general
                    pasteBoard.string = headerView.etcPrivateKeyLbl.text
                    weakSelf.showMessage(MRLocalizableStringManager.localizableStringFromTableHandle(key: MR_copyed_prompt_text_key))
                }
            }
